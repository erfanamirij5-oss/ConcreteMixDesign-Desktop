...
type AggregateComplianceSource = {
  material_id?: string; name?: string; material_type?: string; aggregate_role?: string | null; standard?: string; status?: string;
  gradation?: { status?: string; checked_sieve_count?: number; failure_count?: number; fineness_modulus?: number | null; };
  fines_75um?: { status?: string; astm_c117_percent?: number | null; limit_percent?: number | null; };
  physical_properties?: { data_complete?: boolean; ssd_specific_gravity?: number | null; absorption_percent?: number | null; rodded_unit_weight_kg_m3?: number | null; };
  abrasion?: { status?: string; method?: string | null; loss_percent?: number | null; limit_percent?: number | null; };
  soundness?: { status?: string; salt?: string | null; loss_percent?: number | null; limit_percent?: number | null; };
  deleterious_materials?: { clay_lumps?: { status?: string; percent?: number | null; limit_percent?: number | null; }; lightweight_particles?: { status?: string; percent?: number | null; limit_percent?: number | null; }; };
  shape_texture?: {
    status?: string;
    flat_elongated?: { status?: string; percent?: number | null; limit_percent?: number | null; dimensional_ratio?: string | null; };
    fractured_particles?: { status?: string; percent?: number | null; minimum_percent?: number | null; fractured_faces_required?: number | null; };
    placement_advisory?: { status?: string; message?: string; };
  };
  fractured_face_percent?: number | null; evidence_ref?: string | null;
};
...
export function EngineeringSystemsResults(props: Props) {
  ...
  return <>
    ...
    <article className="panel wide-panel">
      <div className="panel-head"><div><h3>کنترل کیفیت، دوام و شکل سنگدانه‌ها</h3><span>C33/C136/C117/C127/C128/C29 + C131/C535/C88/C142/C123 + D4791/D5821</span></div><span className={`badge ${aggregate?.status === 'fail' ? 'red' : aggregate?.status === 'pass' ? 'green' : 'orange'}`}>{aggregate?.status ?? 'not_checked'}</span></div>
      <div className="panel-body"><div className="result-grid"><div><label>تعداد منابع</label><strong>{aggregateSources.length}</strong></div><div><label>کامل بودن داده‌های اصلی</label><strong>{aggregate?.data_complete ? 'کامل' : 'ناقص'}</strong></div><div><label>منابع مردود</label><strong>{aggregateSources.filter(item => item.status === 'fail').length}</strong></div><div><label>منابع نیازمند بررسی</label><strong>{aggregateSources.filter(item => item.status === 'needs_review').length}</strong></div></div>
        <div className="standards-list">{aggregateSources.length === 0 && <div className="alert warn">هیچ نتیجه Aggregate Compliance از موتور دریافت نشده است.</div>}{aggregateSources.map((item, index) => <div key={`${item.material_id ?? 'aggregate'}-${index}`}><b>{item.name ?? `Aggregate ${index + 1}`}</b> — {item.status ?? '-'} | {item.standard ?? 'ASTM C33/C33M-24a'} | دانه‌بندی: {item.gradation?.status ?? '-'} ({item.gradation?.checked_sieve_count ?? 0} الک، {item.gradation?.failure_count ?? 0} خارج از حد){item.material_type === 'fine_aggregate' ? ` | FM: ${show(item.gradation?.fineness_modulus)}` : ''} | C117: {show(item.fines_75um?.astm_c117_percent)}% / حد {show(item.fines_75um?.limit_percent)}% | SG SSD: {show(item.physical_properties?.ssd_specific_gravity)} | جذب: {show(item.physical_properties?.absorption_percent)}% | C29: {show(item.physical_properties?.rodded_unit_weight_kg_m3)} kg/m³ | LA: {item.abrasion?.status ?? 'not_checked'} {show(item.abrasion?.loss_percent)}% / حد {show(item.abrasion?.limit_percent)}% | C88: {item.soundness?.status ?? 'not_checked'} {show(item.soundness?.loss_percent)}% / حد {show(item.soundness?.limit_percent)}% | C142: {item.deleterious_materials?.clay_lumps?.status ?? 'not_checked'} {show(item.deleterious_materials?.clay_lumps?.percent)}% / حد {show(item.deleterious_materials?.clay_lumps?.limit_percent)}% | C123: {item.deleterious_materials?.lightweight_particles?.status ?? 'not_checked'} {show(item.deleterious_materials?.lightweight_particles?.percent)}% / حد {show(item.deleterious_materials?.lightweight_particles?.limit_percent)}% | D4791: {item.shape_texture?.flat_elongated?.status ?? 'not_checked'} {show(item.shape_texture?.flat_elongated?.percent)}% / حد {show(item.shape_texture?.flat_elongated?.limit_percent)}% ({item.shape_texture?.flat_elongated?.dimensional_ratio ?? '-'}) | D5821: {item.shape_texture?.fractured_particles?.status ?? 'not_checked'} {show(item.shape_texture?.fractured_particles?.percent)}% / حداقل {show(item.shape_texture?.fractured_particles?.minimum_percent)}%، {show(item.shape_texture?.fractured_particles?.fractured_faces_required)} وجه{item.evidence_ref ? ` | گزارش: ${item.evidence_ref}` : ''}<div className={`alert ${item.shape_texture?.placement_advisory?.status === 'review_required' ? 'danger' : item.shape_texture?.placement_advisory?.status === 'acceptable_input' ? 'ok' : 'warn'}`}>کارایی/پمپاژ: {item.shape_texture?.placement_advisory?.message ?? 'داده شکل و بافت برای ارزیابی ثبت نشده است.'}</div></div>)}{aggregate?.status === 'needs_review' && <div className="alert warn">یکی از حدود پروژه یا داده‌های اصلی سنگدانه ناقص است. برای کنترل‌های شکل و بافت نیز بدون حد Specification، Pass قطعی یا تصحیح عددی کارایی اعمال نمی‌شود.</div>}</div>
      </div>
    </article>
