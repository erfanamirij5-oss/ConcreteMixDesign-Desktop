...
export function EngineeringSystemsResults(props: Props) {
  ...
  return <>
    ...
    <article className="panel wide-panel">
      <div className="panel-head"><div><h3>کنترل کیفیت و دوام سنگدانه‌ها</h3><span>C33/C33M-24a + C136/C136M + C117 + C127/C128 + C29 + C131/C535 + C88 + C142 + C123</span></div><span className={`badge ${aggregate?.status === 'fail' ? 'red' : aggregate?.status === 'pass' ? 'green' : 'orange'}`}>{aggregate?.status ?? 'not_checked'}</span></div>
