...
type Analytics = { status?: string; overall?: Stats; points?: Array<unknown>; method?: { version?: string; acceptanceCriteriaApplied?: boolean; passFailApplied?: boolean; standardComplianceInferred?: boolean }; error?: string };
...
export function EngineeringDecisionSummaryPanel({ mixDesignId }: { mixDesignId: string | null }) {
  ...
  const evidenceCount = analytics?.points?.length ?? stats?.count ?? 0;

  return <section className="panel form-panel">
    <div className="panel-head"><div><h3>Engineering Decision Summary</h3><span>Read-only factual synthesis · no recommendation · no acceptance inference</span></div><button className="btn" disabled={busy} onClick={() => void refresh()}>{busy ? 'در حال بروزرسانی...' : 'بروزرسانی'}</button></div>
    <div className="panel-body">
      ...
      <div className="manager-summary-grid">
        ...
        <div><small>نتایج مقاومت ثبت‌شده</small><strong>{fmt(evidenceCount, 0)}</strong></div>
        <div><small>میانگین مقاومت ثبت‌شده</small><strong>{stats?.mean == null ? '—' : `${fmt(stats.mean)} MPa`}</strong></div>
        <div><small>Sample CV</small><strong>{stats?.sampleCoefficientOfVariationPercent == null ? '—' : `${fmt(stats.sampleCoefficientOfVariationPercent)}%`}</strong></div>
      ...
      <div className="table-wrap"><table><thead><tr><th>Signal</th><th>Observed state</th><th>Traceability</th></tr></thead><tbody>
        <tr><td>Production evidence</td><td>{batchCount > 0 ? `${batchCount} Batch موجود` : 'Batch ثبت نشده'}</td><td>Revision-bound persisted records</td></tr>
        <tr><td>Strength evidence</td><td>{evidenceCount > 0 ? `${evidenceCount} نتیجه · Mean ${fmt(stats?.mean)} MPa` : 'نتیجه مقاومت ثبت نشده'}</td><td>{analytics?.method?.version ?? 'Production QC analytics'}</td></tr>
      ...
      <div className="alert info">این پنل فقط شواهد موجود را تجمیع می‌کند. Acceptance criteria، Pass/Fail، استاندارد compliance و recommendation engine اعمال نشده است.</div>
    </div>
  </section>;
}
