...
type ActiveView = 'dashboard' | 'new-project' | 'materials' | 'gradation';

type EngineResult = { status?: string; engine?: string; version?: string; message?: string; mix_proportions?: { water_kg_m3?: number; cementitious_kg_m3?: number; w_cm_ratio?: number }; };
...
const initialProject: ProjectIntake = {
  ...
  mixDesign: { concreteType: 'normal_weight', targetStrengthMpa: 35, requiredSlumpMm: 100, maxAggregateSizeMm: 19, exposureSummary: 'شرایط دوام در مرحله بعد با کلاس مواجهه ACI 318 تکمیل شود.' }
};
...
export function App() {
  ...
  const kpis = useMemo(() => [
    ...
    { label: 'طرح فعال', value: activeMixDesignId ? 'انتخاب شد' : 'ندارد', hint: 'برای مصالح و دانه‌بندی لازم است', tone: activeMixDesignId ? 'purple' : 'red' }
  ...
  return <div className="app-shell">
    <header className="header"><div className="header-top"><div className="brand"><div className="brand-icon">ط</div><div><strong>طلوع بتن</strong><small>TOLOU CONCRETE MIX DESIGN</small></div></div><div className="module-title"><h1>نرم‌افزار جامع طرح اختلاط انواع بتن</h1><p>طراحی، کنترل دوام، تحلیل دانه‌بندی، گزارش PDF و مگاپرامپت مهندسی</p></div><div className="header-actions"><button>راهنما</button><button>گزارش</button></div></div><div className="header-bottom"><span>ConcreteMixDesign-Desktop / فاز اجرایی اولیه</span><div className="badges"><span className="badge green">Python Engine</span><span className="badge blue">SQLite</span><span className="badge orange">نسخه 0.1.0</span></div></div></header>
    <nav className="top-nav"><button className={activeView === 'dashboard' ? 'active' : ''} onClick={() => setActiveView('dashboard')}>داشبورد</button><button className={activeView === 'new-project' ? 'active' : ''} onClick={() => setActiveView('new-project')}>پروژه جدید</button><button className={activeView === 'materials' ? 'active' : ''} onClick={() => setActiveView('materials')}>مصالح</button><button className={activeView === 'gradation' ? 'active' : ''} onClick={() => setActiveView('gradation')}>دانه‌بندی</button><button>دوام</button><button>نتایج</button><button>گزارش</button></nav>
    <div className="page-grid"><aside className="sidebar"><div className="sidebar-title">ساختار نرم‌افزار</div>{modules.map((item, index) => <button className={sidebarClass(index, activeView)} key={item}><span><b className="ico">{index + 1}</b>{item}</span><span>›</span></button>)}<div className="note"><b>اصل مهندسی</b><br />هیچ خروجی بدون استاندارد، فرضیه، هشدار و قابلیت ردیابی معتبر نیست.</div></aside>
      <main className="workspace">{activeView === 'dashboard' && <Dashboard kpis={kpis} engineState={engineState} engineResult={engineResult} engineError={engineError} recentProjects={recentProjects} onCheckEngine={checkEngine} onNewProject={() => setActiveView('new-project')} onSelectProject={(id) => { setActiveMixDesignId(id); setActiveView('materials'); }} />}{activeView === 'new-project' && <ProjectForm projectIntake={projectIntake} saveStatus={saveStatus} saveMessage={saveMessage} onUpdate={updateProject} onSave={saveProject} />}{activeView === 'materials' && <MaterialsView mixDesignId={activeMixDesignId} />}{activeView === 'gradation' && <GradationView mixDesignId={activeMixDesignId} />}</main>
    </div>
  </div>;
}
