...
type ActiveView = 'dashboard' | 'new-project' | 'materials' | 'gradation' | 'aggregate-blend' | 'durability' | 'results';
...
type EngineResult = {
  ...
  mix_proportions?: {
    water_kg_m3?: number; cementitious_kg_m3?: number | null; w_cm_ratio?: number | null; fine_aggregate_kg_m3?: number | null; coarse_aggregate_kg_m3?: number | null;
  ...
  aggregate_compliance?: EngineeringSystemsProps['aggregateCompliance'];
  combined_aggregate_system?: AggregateBlendResultProps['combined'];
  aggregate_blend_optimizer?: AggregateBlendResultProps['optimizer'];
  ...
  cementitious_system?: EngineeringSystemsProps['cementitiousSystem'];
  cementitious_compliance?: EngineeringSystemsProps['cementitiousCompliance'];
  asr_compliance?: EngineeringSystemsProps['asrCompliance'];
  water_compliance?: EngineeringSystemsProps['waterCompliance'];
  admixture_system?: EngineeringSystemsProps['admixtureSystem'];
  admixture_compliance?: EngineeringSystemsProps['admixtureCompliance'];
  chloride_compliance?: EngineeringSystemsProps['chlorideCompliance'];
};
...
function ResultsView(props: { mixDesignId: string | null; result: EngineResult | null; state: CalculationState; error: string | null; onCalculate: () => void; }) {
  ...
  return <><section className="titlebar"><div><h2>نتایج محاسبات طرح اختلاط</h2><p>محاسبه مستقیم از پروژه، دوام، مصالح و سهم‌های ذخیره‌شده در SQLite توسط Python Engine</p></div><div className="toolbar"><button className="btn success" disabled={!props.mixDesignId || props.state === 'calculating'} onClick={props.onCalculate}>{props.state === 'calculating' ? 'در حال محاسبه...' : 'محاسبه طرح اختلاط'}</button></div></section>
    ...
    {props.result && <>{props.result.status === 'fail' && <div className="alert danger">محاسبه مهندسی با وضعیت FAIL پایان یافته است؛ مقادیر نمایش‌داده‌شده تا رفع خطاهای حاکم برای تولید قابل تأیید نیستند.</div>}<section className="kpis"><article className="kpi blue"><label>وضعیت محاسبه</label><strong>{props.result.status ?? '-'}</strong><small>Engine {props.result.engine_version ?? props.result.version ?? '0.3.0'}</small></article><article className="kpi green"><label>آب طراحی</label><strong>{displayValue(mix?.water_kg_m3)}</strong><small>kg/m³</small></article><article className="kpi orange"><label>مواد سیمانی</label><strong>{displayValue(mix?.cementitious_kg_m3)}</strong><small>kg/m³</small></article><article className="kpi purple"><label>آب قابل تزریق</label><strong>{displayValue(mix?.water_to_add_kg_m3)}</strong><small>kg/m³ پس از رطوبت و افزودنی</small></article></section>
      ...
      <EngineeringSystemsResults cementitiousSystem={props.result.cementitious_system} cementitiousCompliance={props.result.cementitious_compliance} aggregateCompliance={props.result.aggregate_compliance} asrCompliance={props.result.asr_compliance} waterCompliance={props.result.water_compliance} admixtureSystem={props.result.admixture_system} admixtureCompliance={props.result.admixture_compliance} chlorideCompliance={props.result.chloride_compliance} waterToAddKgM3={mix?.water_to_add_kg_m3} />
