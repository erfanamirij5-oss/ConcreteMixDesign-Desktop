...
export type ReportSnapshot = {
  ...
  gradation: Array<Record<string, unknown>>;
  durability: Record<string, unknown> | null;
  blend: Record<string, unknown>;
  calculation: Record<string, unknown> | null;
  trialMix: Array<Record<string, unknown>>;
  standards: string[];
...
export function createReportSnapshotInDatabase(
  ...
  input: { mixDesignId: string; reportType: ReportType; language: ReportLanguage; generatedBy?: string }
) {
  ...
  const snapshot = buildCanonicalSnapshot(database, input.mixDesignId, input.reportType, input.language, generatedAt, identity, revisionNumber);
  const id = crypto.randomUUID();

  database.prepare(`
    INSERT INTO report_snapshots (
      id, mix_design_id, revision_number, report_type, language,
      snapshot_json, generated_by, generated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    id,
    input.mixDesignId,
...
export function getReportSnapshotFromDatabase(database: Database.Database, id: string) {
  const row = database.prepare(`
    SELECT id, mix_design_id AS mixDesignId, revision_number AS revisionNumber,
      report_type AS reportType, language, snapshot_json AS snapshotJson,
...
export function listReportSnapshotsFromDatabase(database: Database.Database, mixDesignId: string) {
  return database.prepare(`
    SELECT id, mix_design_id AS mixDesignId, revision_number AS revisionNumber,
      report_type AS reportType, language, generated_by AS generatedBy,
...
function buildCanonicalSnapshot(
  ...
  const materialIds = materials.map(item => String(item.id));
  const gradation = materialIds.length
    ? rows(database, `SELECT s.*, m.name AS material_name, m.material_type FROM aggregate_sieve_results s INNER JOIN materials m ON m.id = s.material_id WHERE m.mix_design_id = ? ORDER BY m.name, s.sieve_size_mm DESC`, mixDesignId)
    : [];
  ...
  const standards = uniqueStrings([
    String(identity.designStandard ?? ''),
    String(identity.standardsVersion ?? ''),
    ...asStringArray(trace.standardReferences)
  ]);
  ...
  return {
    ...
    standards,
    signatures: {
      preparedBy: nullableString(identity.designerFullName),
      reviewedBy: null,
      approvedBy: null
    }
  };
...
function getIdentity(database: Database.Database, mixDesignId: string) {
  const row = database.prepare(`
    SELECT md.id AS mixDesignId, md.status, md.revision_number AS revisionNumber,
      ...
      md.exposure_summary AS exposureSummary, md.design_standard AS designStandard,
      md.engineer_notes AS engineerNotes, md.engine_version AS engineVersion,
      md.standards_version AS standardsVersion, md.created_at AS createdAt, md.updated_at AS updatedAt,
      p.project_name AS projectName, p.city, p.location_description AS locationDescription,
      p.structure_type AS structureType, p.element_type AS elementType, p.client_name AS clientName,
      p.contractor_name AS contractorName, p.consultant_name AS consultantName,
      l.lab_name AS laboratoryName, l.license_number AS laboratoryLicenseNumber,
      l.address AS laboratoryAddress, l.phone AS laboratoryPhone, l.logo_path AS laboratoryLogoPath,
      d.full_name AS designerFullName, d.role AS designerRole,
...
function nullableString(value: unknown) { const text = typeof value === 'string' ? value.trim() : ''; return text || null; }
