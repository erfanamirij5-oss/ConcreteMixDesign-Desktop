...
export async function createValidatedBackup(database: Database.Database, destination: string): Promise<BackupManifest> {
  ...
  const manifest = buildManifest(temp, new Date().toISOString(), path.resolve(database.name));
...
export function validateBackupCandidate(candidate: string): BackupManifest {
  ...
  const actual = buildManifest(candidate, manifest.createdAt, manifest.sourceDatabasePath);
...
function buildManifest(filePath: string, createdAt: string, sourceDatabasePath: string): BackupManifest {
  const database = new Database(filePath, { readonly: true, fileMustExist: true });
  ...
  return { formatVersion: 1, createdAt, sourceDatabasePath, schemaMigrations, sha256: crypto.createHash('sha256').update(bytes).digest('hex'), sizeBytes: statSync(filePath).size };
}
...
function readAndValidateManifest(candidate: string): BackupManifest {
  ...
  if (typeof value.sourceDatabasePath !== 'string' || !value.sourceDatabasePath.trim()) throw new Error('Backup manifest source database path is invalid.');
