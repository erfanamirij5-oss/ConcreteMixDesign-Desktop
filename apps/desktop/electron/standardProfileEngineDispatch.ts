...
export const LEGACY_V11_PROFILE: StandardProfile = {
  ...
  references: [
    {
      authority: 'ACI',
      designation: 'PRC-211.1',
      edition: '2022',
      title: 'Selecting Proportions for Normal-Density and High-Density Concrete',
    },
    {
      authority: 'ACI',
      designation: 'CODE-318',
      edition: '2025',
...
export function prepareEnginePayloadForCommand(command: string, payload?: unknown): unknown {
  ...
  const resolved = resolveStandardProfileChain(
    [LEGACY_V11_PROFILE],
    LEGACY_V11_PROFILE.profileId,
    LEGACY_V11_PROFILE.profileVersion,
  );
  ...
  const standardProfile: EngineStandardProfileEnvelope = {
    ...
    chain: resolved.chain.map(item => ({
      profile_id: item.profileId,
      profile_version: item.profileVersion,
      scope: item.scope,
    })),
  ...
  return {
    ...input,
    standards: [...LEGACY_V11_ENGINE_STANDARD_IDS],
