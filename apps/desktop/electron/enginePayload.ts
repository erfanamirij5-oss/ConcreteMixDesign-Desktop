...
export function buildNormalMixPayload(mixDesignId: string) {
  ...
  for (const aggregate of aggregates) {
    ...
    const rows = database.prepare(`
      ...
      FROM aggregate_sieve_results
      WHERE material_id = ?
      ORDER BY sieve_size_mm DESC
  ...
  return {
    ...
    standards: ['ACI_PRC_211_1_22', 'ACI_318', 'ACI_301', 'ASTM_C29', 'ASTM_C33', 'ASTM_C127', 'ASTM_C128', 'ASTM_C136'],
    materials: {
      aggregates,
