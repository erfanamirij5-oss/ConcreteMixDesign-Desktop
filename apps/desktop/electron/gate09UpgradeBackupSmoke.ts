...
async function run() {
  try {
    ...
    assert.equal(migrations.length, 23, 'Gate08 upgrade must append security migration 022 and Trial Mix v2 foundation migration 023 exactly once');
    assert.ok(migrations.some(item => item.id === '022_users_roles_audit_security'), 'Gate09 security migration 022 must remain present after later runtime migrations');
    ...
    assert.ok(manifest.schemaMigrations.includes('022_users_roles_audit_security'));
    ...
    console.log('Gate 09 upgrade/backup smoke passed: representative Gate08 schema upgrades through 023 while Gate09 users, password verifiers, roles and append-only audit survive validated backup/restore.');
  } finally {
