...
const saved = saveTrialMixRecordToDatabase(database, {
  ...
  actualSlumpMm: 95,
  airContentPercent: 2.1,
  concreteTemperatureC: 27.5,
  freshDensityKgM3: 2385,
  strength7dMpa: 31.4,
  strength28dMpa: 43.8,
  notes: 'Reference minimum Trial Mix record',
  actorName: 'CI Smoke'
});
...
if (records[0].actualSlumpMm !== 95 || records[0].strength28dMpa !== 43.8) throw new Error('Trial Mix persisted values were not reopened faithfully.');

const audit = database.prepare("SELECT action, actor_name AS actorName FROM audit_logs WHERE mix_design_id = 'mix-1'").get() as { action: string; actorName: string };
...
database.close();
console.log('Minimum Trial Mix smoke passed: persistence, validation, reopen fidelity, audit and workflow guards are enforced.');
