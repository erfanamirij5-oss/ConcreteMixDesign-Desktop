...
export function addMaterialTestObservation(database: Database.Database, input: MaterialTestObservationInput) {
  ...
  database.prepare(`INSERT INTO material_test_observations (
    ...
    .run(
      id,
      input.materialLibraryId,
      input.observedAt,
      input.propertyKey.trim(),
      input.numericValue ?? null,
      input.textValue?.trim() || null,
      input.unit?.trim() || null,
      input.methodReference?.trim() || null,
      input.standardEdition?.trim() || null,
      input.laboratoryName?.trim() || null,
      input.reportNumber?.trim() || null,
      input.evidenceRef?.trim() || null,
      input.sourceKind ?? 'measured',
...
export function addMaterialQualificationEvent(database: Database.Database, input: MaterialQualificationEventInput) {
  ...
  database.prepare(`INSERT INTO material_qualification_events (
    ...
    .run(
      ...
      input.status,
      input.basis.trim(),
      input.standardReference?.trim() || null,
      input.standardEdition?.trim() || null,
      input.evidenceRef?.trim() || null,
...
export function listMaterialTestObservations(database: Database.Database, materialLibraryId: string) {
  ...
  return database.prepare(`SELECT
      id,
      material_library_id AS materialLibraryId,
      observed_at AS observedAt,
      property_key AS propertyKey,
      numeric_value AS numericValue,
      text_value AS textValue,
      unit,
      method_reference AS methodReference,
      standard_edition AS standardEdition,
    ...
    FROM material_test_observations
    WHERE material_library_id = ?
    ORDER BY observed_at DESC, created_at DESC`).all(materialLibraryId);
}
...
export function listMaterialQualificationEvents(database: Database.Database, materialLibraryId: string) {
  ...
  return database.prepare(`SELECT
      ...
      status,
      basis,
      standard_reference AS standardReference,
      standard_edition AS standardEdition,
      evidence_ref AS evidenceRef,
      actor,
      reason,
      created_at AS createdAt
    FROM material_qualification_events
    WHERE material_library_id = ?
    ORDER BY event_at DESC, created_at DESC`).all(materialLibraryId);
}

export function addMaterialObservation(input: MaterialTestObservationInput) {
  return addMaterialTestObservation(getDatabase(), input);
}

export function addMaterialQualification(input: MaterialQualificationEventInput) {
  return addMaterialQualificationEvent(getDatabase(), input);
}

export function listMaterialObservations(materialLibraryId: string) {
  return listMaterialTestObservations(getDatabase(), materialLibraryId);
}

export function listMaterialQualifications(materialLibraryId: string) {
  return listMaterialQualificationEvents(getDatabase(), materialLibraryId);
}
