...
def calculate_integrated_normal_mix(payload: dict) -> dict:
    """Run durability first, then apply governing durability limits to mix design."""
    ...
    durability = evaluate_durability(
        {
            "conditions": durability_conditions,
    ...
    if durability_max_w_cm is not None:
        if original_w_cm is None:
            requirements["w_cm_ratio"] = durability_max_w_cm
        else:
            requirements["w_cm_ratio"] = min(float(original_w_cm), float(durability_max_w_cm))
    ...
    if durability_min_strength_mpa is not None and target_strength_mpa < float(durability_min_strength_mpa):
        warnings.append(
            {
                "code": "TARGET_STRENGTH_BELOW_DURABILITY_MINIMUM",
                "severity": "fail",
                "message": (
                    f"مقاومت هدف پروژه {target_strength_mpa:g} MPa از حداقل موردنیاز دوام "
                    f"{float(durability_min_strength_mpa):g} MPa کمتر است."
                ),
    ...
    result["calculation_pipeline"] = ["ACI_318_25_durability", "ACI_PRC_211_1_22_proportioning"]
    return result
