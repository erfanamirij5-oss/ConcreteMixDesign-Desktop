...
Gate 12 may close only when the installer configuration is deterministic, implicit publishing is disabled, the commercial installer contract passes exact-head CI, a Tolou application icon is packaged, no private signing material is present, and no known packaging defect would prevent Gate 13 acceptance testing.
